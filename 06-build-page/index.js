        ...
        fs.readdir(components, {withFileTypes: true}, (err, files) => {
            files.forEach(file => {    
                ...
                component.on('data', (data) => {
                    ...
                    if ( i === mass.length - 1 && el === elem && files[files.length - 1].name === file.name) {
...
fs.mkdir(pageAssets, { recursive: true }, (err) => {
    ...
    fs.readdir(pageAssets, {withFileTypes: true}, (err, files) => {
        ...
        fs.readdir(assets, {withFileTypes: true}, (err, files) => {
            ...
            files.forEach(file => {
                if (file.isFile()) {
                    fs.copyFile(path.join(assets, file), path.join(pageAssets, file), (err) => {
                        if (err) {
                            throw err;
                        }
                    });
                } else {
                    fs.mkdir(path.join(pageAssets, file.name), { recursive: true }, (err) => {
                        if (err) {
                            throw err;
                        }
                        fs.readdir(path.join(assets, file.name), (err, files) => {
                            files.forEach(f => {
                                fs.copyFile(path.join(assets, file.name, f), path.join(pageAssets, file.name, f), (err) => {
                                    if (err) {
                                        throw err;
                                    }
                                });
